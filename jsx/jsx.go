...
func setAEComments(ae *javascript.AssignmentExpression, before, after javascript.Comments) {
	...
	if ae.Yield && ae.AssignmentExpression != nil {
		if before != nil {
			ae.Comments[0] = combineComments(before, ae.Comments[0])
		}

		setAEComments(ae.AssignmentExpression, nil, after)
	} else if ae.ArrowFunction != nil {
		if before != nil {
			ae.ArrowFunction.Comments[0] = combineComments(before, ae.ArrowFunction.Comments[0])
		}

		if ae.ArrowFunction.FunctionBody != nil {
			ae.ArrowFunction.Comments[4] = combineComments(ae.ArrowFunction.Comments[4], after)
		} else if ae.ArrowFunction.AssignmentExpression != nil {
			setAEComments(ae.ArrowFunction.AssignmentExpression, nil, after)
		}
	} else if ae.LeftHandSideExpression != nil && ae.AssignmentExpression != nil {
		setLHSComments(ae.LeftHandSideExpression, before, nil)
		setAEComments(ae.AssignmentExpression, nil, after)
	} else if ae.AssignmentPattern != nil && ae.AssignmentExpression != nil && ae.AssignmentOperator == javascript.AssignmentAssign {
		ae.AssignmentPattern.Comments[0] = combineComments(before, ae.AssignmentPattern.Comments[0])
		setAEComments(ae.AssignmentExpression, nil, after)
	} else if ae.ConditionalExpression != nil {
		setCEComments(ae.ConditionalExpression, before, after)
	}
}

func combineComments(a, b javascript.Comments) javascript.Comments {
	if a == nil {
		return b
	} else if b == nil {
		return a
...
func setCEComments(ce *javascript.ConditionalExpression, before, after javascript.Comments) {
	if ce.LogicalORExpression != nil {
		setUnaryExpressionComments(getFirstUE(ce.LogicalORExpression), before, nil)

		if ce.False == nil {
			setUpdateExpressionComments(&ce.LogicalORExpression.LogicalANDExpression.BitwiseORExpression.BitwiseXORExpression.BitwiseANDExpression.EqualityExpression.RelationalExpression.ShiftExpression.AdditiveExpression.MultiplicativeExpression.ExponentiationExpression.UnaryExpression.UpdateExpression, nil, after)
		}
	} else if ce.CoalesceExpression != nil {
		c := ce.CoalesceExpression

		for c.CoalesceExpressionHead != nil {
			c = c.CoalesceExpressionHead
		}

		setUnaryExpressionComments(getFirstUE(&c.BitwiseORExpression), before, nil)

		if ce.False == nil {
			setUpdateExpressionComments(&c.BitwiseORExpression.BitwiseXORExpression.BitwiseANDExpression.EqualityExpression.RelationalExpression.ShiftExpression.AdditiveExpression.MultiplicativeExpression.ExponentiationExpression.UnaryExpression.UpdateExpression, nil, after)
		}
	}

	if ce.False != nil {
		setAEComments(ce.False, nil, after)
	}
}

type ueFinder struct {
	UE *javascript.UnaryExpression
}

func (u *ueFinder) Handle(t javascript.Type) error {
	switch t := t.(type) {
	...
	return walk.Walk(t, u)
}

func getFirstUE(t javascript.Type) *javascript.UnaryExpression {
	var uef ueFinder
...
func setUnaryExpressionComments(ue *javascript.UnaryExpression, before, after javascript.Comments) {
	if len(ue.UnaryOperators) > 0 && before != nil {
		ue.UnaryOperators[0].Comments = combineComments(before, ue.UnaryOperators[0].Comments)
		before = nil
	}

	setUpdateExpressionComments(&ue.UpdateExpression, before, after)
}

func setUpdateExpressionComments(ue *javascript.UpdateExpression, before, after javascript.Comments) {
	switch ue.UpdateOperator {
	case javascript.UpdatePreIncrement, javascript.UpdatePreDecrement:
		ue.Comments = combineComments(before, ue.Comments)
		before = nil
	case javascript.UpdatePostIncrement, javascript.UpdatePostDecrement:
		ue.Comments = combineComments(ue.Comments, after)
		after = nil
	}

	if ue.LeftHandSideExpression != nil {
		setLHSComments(ue.LeftHandSideExpression, before, after)
	} else if ue.UnaryExpression != nil && after != nil {
		setUpdateExpressionComments(&ue.UnaryExpression.UpdateExpression, nil, after)
	}
}

func setLHSComments(lhs *javascript.LeftHandSideExpression, before, after javascript.Comments) {
	if lhs == nil || before == nil && after == nil {
		return
	}

	if lhs.NewExpression != nil {
		if len(lhs.NewExpression.News) > 0 {
			lhs.NewExpression.News[0] = combineComments(before, lhs.NewExpression.News[0])
		}

		setMEComments(&lhs.NewExpression.MemberExpression, nil, after)
	} else if lhs.CallExpression != nil {
		ce := lhs.CallExpression

		for ce.CallExpression != nil {
			ce = ce.CallExpression
		}

		setMEComments(ce.MemberExpression, before, nil)

		lhs.CallExpression.Comments[4] = combineComments(lhs.CallExpression.Comments[4], after)
	} else if lhs.OptionalExpression != nil {
		oe := lhs.OptionalExpression

		for oe.OptionalExpression != nil {
			oe = oe.OptionalExpression
		}

		setMEComments(oe.MemberExpression, before, nil)

		lhs.OptionalExpression.OptionalChain.Comments[3] = combineComments(lhs.OptionalExpression.OptionalChain.Comments[3], after)
	}
}

func setMEComments(me *javascript.MemberExpression, before, after javascript.Comments) {
	if me == nil || before == nil && after == nil {
		return
	}

	me.Comments[4] = combineComments(me.Comments[4], after)

	for me.MemberExpression != nil {
		me = me.MemberExpression
	}

	me.Comments[0] = combineComments(before, me.Comments[0])
}
