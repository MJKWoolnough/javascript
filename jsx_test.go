...
func TestJSXAttribute(t *testing.T) {
	doTests(t, []sourceFn{
		...
		{"<a={await b}/>", func(t *test, tk Tokens) { // 13
			t.Await = true
			t.Output = JSXAttribute{
				Identifier: &tk[1],
				AssignmentExpression: &AssignmentExpression{
		...
		{"<a={await b}/>", func(t *test, tk Tokens) { // 14
			t.Err = Error{
				Err:     ErrMissingClosingBrace,
				Parsing: "JSXAttribute",
				Token:   tk[6],
		...
		{"<a=<b></c>/>", func(t *test, tk Tokens) { // 20
			t.Err = Error{
				Err: Error{
			...
			t.Output = JSXAttribute{
				...
				AssignmentExpression: &AssignmentExpression{
					ConditionalExpression: WrapConditional(&MemberExpression{
						...
						Comments: [5]Comments{{&tk[5], &tk[7]}, nil, nil, nil, {&tk[11], &tk[13]}},
						Tokens:   tk[5:14],
					}),
					Tokens: tk[5:14],
				},
			...
			t.Output = JSXAttribute{
				AssignmentExpression: &AssignmentExpression{
					ConditionalExpression: WrapConditional(&MemberExpression{
						...
						Comments: [5]Comments{{&tk[9]}, nil, nil, nil, {&tk[13], &tk[15]}},
						Tokens:   tk[9:16],
					}),
					Tokens: tk[9:16],
				},
		...
		{"<a /*A*/ = /*B*/ '' // C\n/>", func(t *test, tk Tokens) { // 30
			t.Output = JSXAttribute{
				Identifier: &tk[1],
				JSXString:  &tk[9],
				Comments:   [5]Comments{nil, nil, {&tk[3]}, {&tk[7]}, {&tk[11]}},
				Tokens:     tk[1:12],
			}
		}},
		{"<a /*A*/ : /*B*/ b /*C*/ = /*D*/ '' // E\n/>", func(t *test, tk Tokens) { // 31
			t.Output = JSXAttribute{
				...
				Identifier: &tk[9],
				JSXString:  &tk[17],
				Comments:   [5]Comments{{&tk[3]}, {&tk[7]}, {&tk[11]}, {&tk[15]}, {&tk[19]}},
				Tokens:     tk[1:20],
			}
		}},
	}, func(t *test) (Type, error) {
...
func TestJSXChild(t *testing.T) {
	doTests(t, []sourceFn{
		...
		{"<>{await a}</>", func(t *test, tk Tokens) { // 6
			t.Await = true
			t.Output = JSXChild{
				JSXChildExpression: &AssignmentExpression{
					ConditionalExpression: WrapConditional(&UnaryExpression{
		...
		{"<>{await a}</>", func(t *test, tk Tokens) { // 7
			t.Err = Error{
				Err:     ErrMissingClosingBrace,
				Parsing: "JSXChild",
				Token:   tk[5],
			...
			t.Output = JSXChild{
				JSXChildExpression: &AssignmentExpression{
					ConditionalExpression: WrapConditional(&MemberExpression{
						...
						Comments: [5]Comments{{&tk[4], &tk[6]}, nil, nil, nil, {&tk[10], &tk[12]}},
						Tokens:   tk[4:13],
					}),
					Tokens: tk[4:13],
				},
			...
			t.Output = JSXChild{
				...
				JSXChildExpression: &AssignmentExpression{
					ConditionalExpression: WrapConditional(&MemberExpression{
						...
						Comments: [5]Comments{{&tk[10]}, nil, nil, nil, {&tk[14], &tk[16]}},
						Tokens:   tk[10:17],
					}),
					Tokens: tk[10:17],
				},
				Comments: Comments{&tk[4], &tk[6]},
				Tokens:   tk[2:19],
			}
		}},
